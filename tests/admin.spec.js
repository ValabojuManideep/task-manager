...
test.describe.only('Admin – Performance Leaderboard', () => {
  ...
  test('Team leaderboard: score and ranking validation', async ({ adminPage }) => {
    await adminPage.goto('/leaderboard');
    // If there are teams, check for score and ranking
    const teamItems = adminPage.locator('.leaderboard-item');
    if (await teamItems.count() > 0) {
      // Check rank numbers and score
      const firstRank = teamItems.first().locator('.rank-number');
      await expect(firstRank).toHaveText('#1');
      const score = teamItems.first().locator('.metric-value.score');
