...
export default function TaskList({ statusFilter, priorityFilter, displayTasks }) {
  ...
  const notifiedRecurrentTasks = useAppStore(
    (s) => s.taskList_notifiedRecurrentTasks
  );
  const setNotifiedRecurrentTasks = useAppStore(
    (s) => s.setTaskList_notifiedRecurrentTasks
  );
  ...
  const mentionDropdownSelected = useAppStore(
    (s) => s.taskList_mentionDropdownSelected
  );
  ...
  const handleMarkDone = async (task) => {
    ...
    if (
      ...
      new Date(task.dueDate).toDateString() ===
        new Date(task.recurrenceEndDate).toDateString()
    ) {
      const confirmed = await confirmAction(
  ...
  const handleTaskRowClick = (task, e) => {
    e.stopPropagation();
                    ...
                    <React.Fragment key={id}>
                      ...
                      >
                        ...
                        <td className="task-assigned-cell">
                          {task.isTeamTask ? (
                            <>
                              👥 {task.assignedToTeam?.name || "No team"}{" "}
                        ...
                        <td>
                          {(task.isRecurrent ||
                            ...
                            <span className="task-recurrent-pill">Yes</span>
                          )}
                        </td>
                        <td>
                          <button
                      ...
                      {/* Completion log modal (kept same, positioned absolutely) */}
                      {logTaskId === id && (
                        <tr className="task-log-row">
                          <td colSpan={9}>
                            <div
                                ...
                                <div style={{ position: "relative", display: "flex", gap: "8px", alignItems: "center" }}>
                                  <input
                                    ...
                                    onKeyDown={(e) => {
                                      if (
                                        task.isTeamTask &&
                                        mentionDropdowns[id]?.show
                                      ) {
                                        const teamMembers =
                                          task.assignedToTeam?.members ||
                                          [];
                                        const filteredUsers = users.filter(
                                          (u) =>
                                            ...
                                            u.username
                                              .toLowerCase()
                                              .includes(
                                                mentionDropdowns[id].search.toLowerCase()
                                              )
                                        );
                                        let selectedIdx =
                                          mentionDropdownSelected[id] ?? 0;
                                        if (e.key === "ArrowDown") {
                                          e.preventDefault();
                                    ...
                                    mentionDropdowns[id]?.show && (
                                      <div
                                        style={{
                                          ...
                                          borderRadius: "10px",
                                          boxShadow:
                                            "0 8px 32px rgba(91,127,255,0.25)",
                                          zIndex: 9999,
                                          minWidth: "200px",
                                          maxHeight: "220px",
                                          overflowY: "auto",
...
//   // Render Comments Section (shared by both views)
//   const renderCommentsSection = (task, id) => (
...
//               key={comment._id}
//               className={`comment-item ${
//                 comment.userRole === "admin" ? "admin-comment" : ""
//               }`}
//             >
//               {editingComment === comment._id ? (
//                 <div className="edit-comment-wrapper">
...
//                 setMentionDropdowns((dropdowns) => ({
//                   ...dropdowns,
//                   [id]: { show: false, search: "", pos: { left: 0, top: 40 } },
//                 }));
//               }
//             }}
//             onKeyDown={(e) => {
...
//                   .filter(
//                     (u) =>
//                       (task.assignedToTeam?.members || []).some(
//                         (m) => m._id === u._id
//                       ) &&
//                       u.username
//                         .toLowerCase()
...
//                         cursor: "pointer",
//                         background:
//                           (mentionDropdownSelected[id] ?? 0) === idx
//                             ? "#e6edff"
//                             : "inherit",
//                         color:
//                           (mentionDropdownSelected[id] ?? 0) === idx
...
//                   ))
//               ) : (
//                 <div style={{ padding: "10px 16px", color: "#999" }}>
//                   No team members found
//                 </div>
//               )}
//             </div>
...
//   // Render Completion Log Modal (shared by both views)
//   const renderCompletionLogModal = (task) => (
//     <div
//       className="completion-log-modal"
//       onClick={(e) => e.stopPropagation()}
//       style={{
...
//                         {/* Completion log modal for list view */}
//                         {logTaskId === id && (
//                           <tr className="task-log-row">
//                             <td colSpan={9}>{renderCompletionLogModal(task)}</td>
//                           </tr>
//                         )}

//                         {/* Comments row for list view */}
//                         {isExpanded && (
//                           <tr className="task-comments-row">
//                             <td colSpan={9}>{renderCommentsSection(task, id)}</td>
//                           </tr>
//                         )}
//                       </React.Fragment>
//                     );
...
//                         (task.recurrencePattern &&
//                           task.recurrencePattern !== "none")) && (
//                         <span className="task-recurrent-label">Recurrent</span>
//                       )}
//                     </div>
...
//                     {/* Completion log modal for cards */}
//                     {logTaskId === id && renderCompletionLogModal(task)}

//                     {/* Comments section for cards */}
//                     {isExpanded && renderCommentsSection(task, id)}
//                   </div>
//                 );
//               })}
//             </div>
